...
class DB_CONNECTION	{
			...
			public static function get_event_categories() {
								...
								$sql = "CALL availableEventCatagories()";
		...
		public static function search_Event($searchText){
			$sql = "CALL searchEvent('".$searchText."')";
		...
		public static function check_email_availablity($input) {
				$connection = new DB_CONNECTION();
			$sql = "SELECT email_exists('".trim($input) ."') as 'result' ";
			 $statement = $connection->set_query($sql);
			...
			public function get_event_summary( $organizerId, $eventId){
				$events;
				 $events['tickets'] = null;
				 $events['guests'] = null;
				 $events['sponsors'] = null;
				 $events['comments'] = null;
		$sql = "CALL getEventSummary(".$organizerId.",".$eventId.")";
