...
abstract class Organization implements Event_interface {
	...
	private $billing_number;
			...
			public function set_phone_count($value){
				return $this->phoneCount = $value ;
			}

			public function get_phone_count($value) {
			...
			public function set_social($value) {
			...
			public function get_social() {
				return (isset($this->SOCIALS)) ? $this->SOCIALS : NULL;
			}
			...
			public function get_phone_number($index) {
				return (count($this->phoneNumbers) > $index ) ? $this->phoneNumbers[$index] : trigger_error("Index out of bound for phone number", E_USER_ERROR);
			}
		...
		public function update_billing_address( $service_provider, $mobile_number){
			...
			public function update_website($url){
					...
					if($error == 0){
						...
						try {
							...
							$sql = "updatOrganizationWebsite(".$this->get_id().",".$this->get_website().")";
			...
			public function set_address($value) {
				...
				for($i = 0; $i < count($value); $i++){
					...
					if( isset($value[$i]["country"]["name"])
						&& isset($value[$i]["city"])
						&& isset($value[$i]["subCity"])
						&& isset($value[$i]["location"])
					){

						$this->ADDRESS[$i] = $value[$i];
						$this->set_address_count($this->get_address_count() + 1);

					} else {
						$error = false;
						trigger_error("Required key value pair missing from company address", E_USER_ERROR);
					}

					if(!isset($value[$i]["longitude"])){
						trigger_error("longitude is not provided", E_USER_NOTICE);
					}

					if(!isset($value[$i]["latitude"])) {
							trigger_error("latitude is not provided", E_USER_NOTICE);
					}

				 	if(isset($value[$i]["country"]["abriv"])){
							trigger_error("country abrivation is not provided", E_USER_NOTICE);
					}
			...
			public function update_phone_number($numbers) {
					$error = 0;
						$this->set_phone_number($numbers);
						$phones = [];

						for($i = 0; $i < $this->get_phone_count(); $i++) {
							$phones[$i] = $this->get_phone_number($i);

						}

								try{
									$sql = "CALL updateOrganizationPhone(". $this->get_id() .", ".$this->get_organization_id().",'".json_encode($phones)."')";
			...
			public function set_phone_number($value) {
								...
								for($i = 0 ; $i < count($value); $i++) {
									if(!isset($value[$i]["number"]) || !isset($value[$i]["type"])) {
										trigger_error("JSON data for phone number missing required keys", E_USER_ERROR) ;
										exit;
									} else if (!isset($value[$i]["icon"])){
										trigger_error("JSON data for phone number missing icon", E_USER_NOTICE) ;
									}
									$this->phoneNumbers[$i] = $value[$i];
									$this->set_phone_count($this->get_phone_count() + 1);
								}
			...
			public function update_socialMedia_address($value){

			$error = 0;
				$this->set_social($value);
							if(!$socialAddress = $this->get_social()){
								$error = 1;
							}
		...
		public function add_event_ticket(Event  $event){
		 ...
		 public function update_event_ticket(Event  $event){
			...
			public function add_event(Event $new_event){
					...
					if((! $event["eventCategory"] =  $this->get_event($last_index)->get_category() ) && $error = 1 )
			...
			return $this->gender = ucfirst( $value);
			}
			...
			public function update_event_discription(Event $event){
				$eventIndex = $this->set_event($event);
				$error = 0;
				$discription = null;
